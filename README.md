...
шаблонную строку при передаче пути. Для таких задач есть дополнительные пропсы,
по умолчанию имеющиеся у компонента, который был зарендерен через `<Route/>`:

1. `нistory` - для работы с историей;
2. `location` -описывает текущий путь в адресной строкe;
3. `match` - отслеживает совпадениетекущего маршрута и url
...
- url - то, что написано в адресной строке. Используется для создания вложенных
  маршрутов

  Получим доступ к пропу this.props.match
