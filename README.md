...
**`exact`**. В этом случае **`activeClassName`** применится только к той ссылки,
у которой значение пропа **`to === path`**

### Компонент Switch

_Если в навигации нет адреса, введенного в url-строку, **ничего не
зарендерится**._ **Для обработки запросов по несуществующему адресу: **
